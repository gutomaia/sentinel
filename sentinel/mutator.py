...
class Mutator(ast.NodeTransformer):
    ...
    def visit_If(self, node):
        ...
        if gene == self.gene:
            true = ast.Name(self.mutation, ast.Load())
            #print 'Mutate If "%s" line %d  col %d level %d' % (node, node.lineno, node.col_offset, self.level)
            return ast.If(true, node.body, node.orelse)
        self.level -= 1
        return node
